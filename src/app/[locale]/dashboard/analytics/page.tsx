    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8 stagger-children">
        {summaryStats.map((stat) => (
          <div key={stat.name} className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6 hover:shadow-lg transition-all duration-300">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm font-medium text-slate-500">{stat.name}</p>
                <p className={`mt-2 text-3xl font-bold bg-gradient-to-r ${stat.color} bg-clip-text text-transparent`}>{stat.value}</p>
              </div>
      ...
      <div className="mb-6">
        <div className="flex gap-1 p-1 bg-slate-100 rounded-xl w-fit">
          <button onClick={() => setActiveTab('usage')} className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'usage' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}>
            <span className="flex items-center gap-2">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
              {t('tabUsage')}
            </span>
          </button>
          <button onClick={() => setActiveTab('conversations')} className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'conversations' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}>
            <span className="flex items-center gap-2">
      ...
      {activeTab === 'usage' && (
        <>
          <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6 mb-8">
            <h2 className="text-lg font-semibold text-slate-900 mb-6">{t('usageLimits')}</h2>
            <div className="space-y-6">
              {usageMeters.map((meter) => (
                <div key={meter.name}>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm font-medium text-slate-700">{meter.name}</span>
                    <span className="text-sm text-slate-500">{typeof meter.current === 'number' ? meter.current.toLocaleString() : meter.current} / {typeof meter.limit === 'number' ? meter.limit.toLocaleString() : meter.limit}</span>
                  </div>
                  <div className="relative h-3 bg-slate-100 rounded-full overflow-hidden">
                    <div className={`absolute inset-y-0 left-0 bg-gradient-to-r ${meter.color} rounded-full transition-all duration-500`} style={{ width: `${Math.min(meter.percent, 100)}%` }} />
                  </div>
                  {meter.percent >= 90 && (
          ...
          {stats.breakdown.length > 0 && (
            <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-200/60">
                <h2 className="font-semibold text-slate-900">{t('usageBreakdown')}</h2>
              </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-slate-50/50">
                      ...
                      <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('colRequests')}</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('colTokens')}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {stats.breakdown.map((item) => (
                      <tr key={item.type} className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-6 py-4"><span className="font-medium text-slate-900 capitalize">{item.type}</span></td>
          ...
          <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-4">
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="flex-[2]">
                <label className="block text-xs font-medium text-slate-500 mb-1.5">{t('searchMessages')}</label>
          ...
          <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
            {convLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 className="font-semibold text-slate-900 mb-1">{t('noConversationsYet')}</h3>
                <p className="text-sm text-slate-500">{t('noConversationsHint')}</p>
              </div>
            ) : (
              <div className="divide-y divide-slate-100">
                {conversations.map((conv) => (
                  <div key={conv.id} className="hover:bg-slate-50/50 transition-colors">
                    <button onClick={() => setExpandedConv(expandedConv === conv.id ? null : conv.id)} className="w-full p-4 flex items-start gap-4 text-left">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold shadow-sm flex-shrink-0" style={{ backgroundColor: conv.chatbot.primaryColor || '#6366f1' }}>{conv.chatbot.name.charAt(0).toUpperCase()}</div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="font-medium text-slate-900">{conv.chatbot.name}</span>
                          <span className="text-xs text-slate-500">{conv.messageCount} {t('messages')}</span>
                          {conv.thumbsUp > 0 && (<span className="flex items-center gap-0.5 text-xs text-emerald-600"><svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path d="M2 10.5a1.5 1.5 0 113 0v6a1.5 1.5 0 01-3 0v-6zM6 10.333v5.43a2 2 0 001.106 1.79l.05.025A4 4 0 008.943 18h5.416a2 2 0 001.962-1.608l1.2-6A2 2 0 0015.56 8H12V4a2 2 0 00-2-2 1 1 0 00-1 1v.667a4 4 0 01-.8 2.4L6.8 7.933a4 4 0 00-.8 2.4z" /></svg>{conv.thumbsUp}</span>)}
                          {conv.thumbsDown > 0 && (<span className="flex items-center gap-0.5 text-xs text-red-500"><svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path d="M18 9.5a1.5 1.5 0 11-3 0v-6a1.5 1.5 0 013 0v6zM14 9.667v-5.43a2 2 0 00-1.106-1.79l-.05-.025A4 4 0 0011.057 2H5.64a2 2 0 00-1.962 1.608l-1.2 6A2 2 0 004.44 12H8v4a2 2 0 002 2 1 1 0 001-1v-.667a4 4 0 01.8-2.4l1.4-1.866a4 4 0 00.8-2.4z" /></svg>{conv.thumbsDown}</span>)}
                        </div>
                        <p className="text-sm text-slate-600 truncate">{conv.preview || t('noPreview')}</p>
                      </div>
                      <div className="flex items-center gap-3 flex-shrink-0">
                        <span className="text-xs text-slate-500">{formatDate(conv.updatedAt)}</span>
                        <svg className={`w-5 h-5 text-slate-400 transition-transform ${expandedConv === conv.id ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                      </div>
                    </button>
                              ...
                              <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 ${msg.role === 'USER' ? 'bg-gradient-to-r from-teal-500 to-teal-600 text-white' : 'bg-white border border-slate-200 text-slate-900'}`}>
                                  ...
                                  {msg.reaction && (
                                    <span className={`flex items-center gap-0.5 ${msg.reaction === 'up' ? 'text-emerald-400' : 'text-red-400'}`}>
                                      {msg.reaction === 'up' ? (
                                        ...
                                        <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path d="M18 9.5a1.5 1.5 0 11-3 0v-6a1.5 1.5 0 013 0v6zM14 9.667v-5.43a2 2 0 00-1.106-1.79l-.05-.025A4 4 0 0011.057 2H5.64a2 2 0 00-1.962 1.608l-1.2 6A2 2 0 004.44 12H8v4a2 2 0 002 2 1 1 0 001-1v-.667a4 4 0 01.8-2.4l1.4-1.866a4 4 0 00.8-2.4z" /></svg>
                                      )}
                                    </span>
                                  )}
                                </div>
                                {msg.role === 'ASSISTANT' && msg.sources && Array.isArray(msg.sources) && msg.sources.length > 0 && (
                                  <div className="mt-2 pt-2 border-t border-slate-200">
                                    <p className="text-xs font-medium text-slate-500 mb-1">{tc('sources')}:</p>
            ...
            {pagination.totalPages > 1 && (
              <div className="px-4 py-3 border-t border-slate-200/60 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button onClick={() => fetchConversations(pagination.page - 1)} disabled={pagination.page === 1} className="px-3 py-1.5 rounded-lg text-sm font-medium text-slate-600 hover:bg-slate-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">{tc('previous')}</button>
                  <button onClick={() => fetchConversations(pagination.page + 1)} disabled={pagination.page === pagination.totalPages} className="px-3 py-1.5 rounded-lg text-sm font-medium text-slate-600 hover:bg-slate-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">{tc('next')}</button>
                </div>
              </div>
            )}
          </div>
