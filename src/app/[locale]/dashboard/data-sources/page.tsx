...
export default function DataSourcesPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="font-semibold text-slate-900">{t('uploadFile')}</h2>
              <p className="text-sm text-slate-500">{t('fileTypes')}</p>
            </div>
          </div>
            ...
            <div className={`relative border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all ${uploading ? 'border-slate-200 bg-slate-50' : 'border-slate-300 hover:border-teal-400 hover:bg-teal-50/50'}`}>
              ...
              {uploading ? (
                ...
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-sm font-medium text-slate-700">{t('clickToUpload')}</span>
                  <span className="text-xs text-slate-500 mt-1">{t('orDragDrop')}</span>
                </div>
              )}
            </div>
          </label>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="font-semibold text-slate-900">{t('addUrl')}</h2>
              <p className="text-sm text-slate-500">{t('addUrlSubtitle')}</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="font-semibold text-slate-900">{t('crawlWebsite')}</h2>
              <p className="text-sm text-slate-500">{t('crawlSubtitle')}</p>
            </div>
          </div>
                  ...
                  <tr key={ds.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-1">
                        ...
                        <button onClick={() => handleDelete(ds.id)} className="p-2 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 transition-colors" title={tc('delete')}>
      ...
      <SiteCrawlerModal isOpen={crawlerModalOpen} onClose={() => setCrawlerModalOpen(false)} onImportComplete={fetchDataSources} />
      <DocumentPreviewModal isOpen={previewModalOpen} onClose={() => { setPreviewModalOpen(false); setPreviewDataSource(null) }} dataSourceId={previewDataSource?.id ?? null} dataSourceName={previewDataSource?.name ?? ''} onContentUpdated={fetchDataSources} />
    </div>
  )
}
