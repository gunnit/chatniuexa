...
must_haves:
  truths:
    ...
    - "Failed profile creation rolls back entire signup (tenant + auth user deleted)"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/app/(auth)/signup/actions.ts"
      ...
      via: "rollback on profile failure"
      pattern: "supabase\\.auth\\.admin\\.deleteUser"
...
<task type="auto">
  ...
  <action>
    Create the signup server action with tenant creation and CRITICAL profile rollback:
    ...
    export async function signup(prevState: any, formData: FormData) {
      ...
      // CRITICAL: Profile is required for tenant isolation - if this fails, rollback everything
      if (authData.user) {
        ...
        if (profileError) {
          ...
          // Rollback everything - user without profile breaks tenant isolation
          await supabase.from('tenants').delete().eq('id', tenant.id)

          // Use admin client to delete auth user (requires service role key)
    ...
    - CRITICAL: Rolls back BOTH tenant AND auth user if profile creation fails
    - Uses admin client with SUPABASE_SERVICE_ROLE_KEY for user deletion
    - Redirects to check-email page on success

    IMPORTANT: The SUPABASE_SERVICE_ROLE_KEY environment variable must be set.
    This is required to delete auth users during rollback.
  </action>
  <verify>
    ...
    - Profile error handling includes: delete tenant AND admin.deleteUser call
    - Uses createAdminClient with SUPABASE_SERVICE_ROLE_KEY
  </verify>
  <done>
    Signup action creates tenant, user, and profile, validates with zod, handles errors with FULL rollback on profile failure
  </done>
</task>
...
7. Profile creation failure triggers FULL rollback (tenant deletion + auth user deletion)
</verification>

<success_criteria>
- Signup form accepts name, email, password with validation
- Signup creates tenant, user, and profile in correct order
- Profile creation failure rolls back tenant AND deletes auth user via admin API
